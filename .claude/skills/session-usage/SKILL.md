...
const llm = createOpenAICompatibleAdapter({
  apiKey: 'sk-xxx',
  model: 'gpt-4o-mini',
  baseURL: 'https://api.openai.com/v1',       // MiniMax: https://api.minimaxi.com/v1
})
```

### Anthropic 协议（Claude / MiniMax Anthropic 兼容等）

```typescript
import { createAnthropicAdapter } from '@stello-ai/session'

const llm = createAnthropicAdapter({
  apiKey: 'sk-ant-xxx',
  model: 'claude-sonnet-4-20250514',
  baseURL: 'https://api.anthropic.com',        // 可选，默认 Anthropic 官方
})
```

两个 SDK（`openai`、`@anthropic-ai/sdk`）是 optional peerDependencies，只装需要的那个。

也可以自行实现 `LLMAdapter` 接口：
...
```typescript
import { createSession, createOpenAICompatibleAdapter, InMemoryStorageAdapter } from '@stello-ai/session'

const llm = createOpenAICompatibleAdapter({
  apiKey: 'sk-xxx', model: 'gpt-4o-mini', baseURL: 'https://api.openai.com/v1',
})
...
// res.usage = { promptTokens, completionTokens }
